...
app = Flask(__name__)
with open('Randomforest_pkl.pkl','rb') as file:
    Randomforest=pickle.load(file)
with open('RandomforestReg_pkl','rb') as file:
    randomforestpickel=pickle.load(file)
...
def predict_crop():
    # Code to process form data and make prediction
    ...
    # Render page2.html with prediction
    min_N, max_N = 0,140
    min_P, max_P = 5, 145
    min_K, max_K = 5, 205
    min_temperature, max_temperature = 8, 43  # Celsius
    min_humidity, max_humidity = 14, 99  # Percentage
    min_ph, max_ph = 3, 9
    min_rainfall, max_rainfall = 20, 298  # mm
    ...
    if 0 in [P, K, temperature, humidity, ph, rainfall]:
        return render_template('page2.html', invalid='invalid input data')
    
    
    elif any([N < min_N, P < min_P, K < min_K,temperature < min_temperature, humidity < min_humidity,ph < min_ph,rainfall < min_rainfall]):
        return render_template('page2.html',mininvalid='one of the data entered is lessthan min range')
    elif any([N > max_N, P > max_P, K > max_K, temperature > max_temperature, humidity > max_humidity, ph > max_ph,rainfall > max_rainfall]):
        return render_template('page2.html',maxinvalid='one of the data entered is greaterthan max range')
        

    else:
        features = np.array([[N, P, K, temperature, humidity, ph, rainfall]])
        predict = Randomforest.predict(features).reshape(1, -1)
        predict = np.array(predict)
        prediction = None  # Assign a default value to prediction
        for inner_array in predict:
            if np.any(inner_array == 1):
                index = np.where(inner_array == 1)[0][0]
                prediction = crop_array[index]
        if prediction !=None:
            return render_template('page2.html', prediction=prediction)
        else:
            return render_template('page2.html',prediction='Mazie')

        
@app.route('/predict_yield', methods=['POST'])
def predict_yield():
    # Code to process yield prediction form data and make prediction
    # Render page4.html with prediction
 ...
 'NADIA': 405,
 'PURULIA': 471}
    Season_dict={'Kharif': 1,'Whole Year': 4,'Autumn': 0,'Rabi': 2,'Summer': 3,'Winter': 5}
...
# Perform label encoding for categorical columns
    ...
    yield_prediction="{:.3f}".format(float(randomforestpickel.predict(input_df)))
    return render_template('page4.html', yield_prediction=yield_prediction)

@app.route('/enter_yield_data', methods=['GET'])
def enter_yield_data():
